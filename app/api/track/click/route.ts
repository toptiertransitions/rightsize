...
export async function GET(req: NextRequest) {
  ...
  // Record click synchronously before redirecting — ensures the write completes
  // even if the serverless function is shut down immediately after the response.
  if (verifyTrackToken(enrollmentId, token) && targetUrl && enrollmentId !== "test") {
    try {
