...
export default async function PlanPage({ searchParams }: PageProps) {
  ...
  if (tenantId === "__all_active__" || tenantId === "__all_archived__" || tenantId === "__all_time__") {
    ...
    const tenantOptions = visibleTenants.map((t) => ({ id: t.id, name: t.name, isArchived: t.isArchived ?? false, isPostMove: t.isPostMove ?? false, address: t.address, city: t.city, state: t.state, zip: t.zip, destAddress: t.destAddress, destCity: t.destCity, destState: t.destState, destZip: t.destZip }));
  ...
  if (!tenantId) {
    ...
    if (isManagerOrAdmin) {
      ...
      const tenantOptions = visibleTenants2.map(t => ({ id: t.id, name: t.name, isArchived: t.isArchived ?? false, isPostMove: t.isPostMove ?? false, address: t.address, city: t.city, state: t.state, zip: t.zip, destAddress: t.destAddress, destCity: t.destCity, destState: t.destState, destZip: t.destZip }));
    ...
    if (sysRole === "TTTStaff") {
      ...
      const tenantOptions = allTenants.map(t => ({ id: t.id, name: t.name, isArchived: t.isArchived ?? false, isPostMove: t.isPostMove ?? false, address: t.address, city: t.city, state: t.state, zip: t.zip, destAddress: t.destAddress, destCity: t.destCity, destState: t.destState, destZip: t.destZip }));
      const serviceNames = serviceList.map(s => s.name);
  ...
  const tenantOptions = (isManagerOrAdmin || isTTTStaff)
    ? allTenants.map(t => ({ id: t.id, name: t.name, isArchived: t.isArchived ?? false, isPostMove: t.isPostMove ?? false, address: t.address, city: t.city, state: t.state, zip: t.zip, destAddress: t.destAddress, destCity: t.destCity, destState: t.destState, destZip: t.destZip }))
