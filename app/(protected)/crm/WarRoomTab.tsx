...
function QuarterlyPlanSection({
  ...
  const [actStats, setActStats] = useState<MonthStat[]>([]);
  const [editing, setEditing] = useState(false);
  const [draft, setDraft] = useState<CompanyPlan | null>(null);
  const [saving, setSaving] = useState(false);
  ...
  function cellClass(val: number, goal: number, monthKey: string) {
    if (monthKey > todayMonthKey) return "text-gray-300";
    if (goal <= 0) return val > 0 ? "bg-green-50 text-green-700" : "text-gray-400";
    if (val >= goal) return "bg-green-100 text-green-700 font-semibold";
    if (val >= goal * 0.66) return "bg-amber-50 text-amber-700";
    return "bg-red-50 text-red-600";
  }

  function statusColor(s: CompareResult["status"]) {
    return s === "done" ? "bg-green-50 border-green-200 text-green-800" :
      s === "in-progress" ? "bg-amber-50 border-amber-200 text-amber-800" :
      "bg-gray-50 border-gray-200 text-gray-600";
  }

  function statusDot(s: CompareResult["status"]) {
    return s === "done" ? "bg-green-500" : s === "in-progress" ? "bg-amber-400" : "bg-gray-300";
  }
    ...
    <div className="bg-white border-t border-slate-200 px-5 pt-4 pb-5">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="text-xs text-indigo-600 hover:text-indigo-800 border border-indigo-200 bg-indigo-50 px-2.5 py-1 rounded-lg disabled:opacity-50"
          >
            {compareLoading ? "Analyzing..." : "Compare Activity vs Plan"}
          </button>
          {!editing && (
            <button onClick={startEdit}
              className="text-xs text-forest-600 hover:text-forest-800 border border-forest-200 bg-forest-50 px-2.5 py-1 rounded-lg">
          ...
          {editing && (
            <>
              <button onClick={savePlan} disabled={saving}
                className="text-xs bg-forest-600 text-white px-2.5 py-1 rounded-lg hover:bg-forest-700 disabled:opacity-50">
                {saving ? "Saving..." : "Save"}
              </button>
      ...
      {editing && draft ? (
        <div className="space-y-4">
          <div>
            ...
            <div className="space-y-1.5">
              {(["meeting1", "meeting2", "meeting3"] as const).map((k, i) => (
                <input key={k} value={draft[k]} onChange={(e) => setDraft({ ...draft, [k]: e.target.value })}
                  placeholder={`Meeting ${i + 1}`}
                  className="w-full h-8 border border-gray-200 rounded-lg px-2.5 text-xs focus:outline-none focus:ring-1 focus:ring-forest-400" />
              ))}
            </div>
          </div>
          <div>
            <p className="text-xs font-medium text-gray-500 mb-1.5">Key Resources</p>
            <div className="space-y-1.5">
              {(["resource1", "resource2", "resource3"] as const).map((k, i) => (
                <input key={k} value={draft[k]} onChange={(e) => setDraft({ ...draft, [k]: e.target.value })}
          ...
          <div className="flex gap-4">
            <div className="flex-1">
              <p className="text-xs font-medium text-gray-500 mb-1">Monthly Meeting Goal</p>
              <input type="number" min="0" value={draft.monthlyMeetingGoal}
                onChange={(e) => setDraft({ ...draft, monthlyMeetingGoal: parseInt(e.target.value) || 0 })}
                className="w-full h-8 border border-gray-200 rounded-lg px-2.5 text-xs focus:outline-none focus:ring-1 focus:ring-forest-400" />
            </div>
            <div className="flex-1">
              <p className="text-xs font-medium text-gray-500 mb-1">Monthly Check-in Goal</p>
              <input type="number" min="0" value={draft.monthlyCheckinGoal}
                onChange={(e) => setDraft({ ...draft, monthlyCheckinGoal: parseInt(e.target.value) || 0 })}
                className="w-full h-8 border border-gray-200 rounded-lg px-2.5 text-xs focus:outline-none focus:ring-1 focus:ring-forest-400" />
            </div>
          </div>
        </div>
        ...
        <>
          {plan && (
            <>
              {(plan.meeting1 || plan.meeting2 || plan.meeting3) && (
                <div className="mb-3">
                  <p className="text-xs font-medium text-gray-500 mb-1">Key Meetings</p>
                  <ul className="space-y-0.5">
                    {[plan.meeting1, plan.meeting2, plan.meeting3].filter(Boolean).map((m, i) => (
                      <li key={i} className="text-xs text-gray-700 flex items-start gap-1.5">
                        <span className="text-gray-300 mt-0.5">•</span>{m}
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              {(plan.resource1 || plan.resource2 || plan.resource3) && (
                <div className="mb-3">
                  <p className="text-xs font-medium text-gray-500 mb-1">Key Resources</p>
                  <ul className="space-y-0.5">
                    {[plan.resource1, plan.resource2, plan.resource3].filter(Boolean).map((r, i) => (
                      <li key={i} className="text-xs text-gray-700 flex items-start gap-1.5">
          ...
          {!plan && (
            <p className="text-xs text-gray-400 italic mb-3">No plan set for this quarter yet.</p>
          )}
        </>
      )}

      {actStats.length > 0 && plan && (
        <div className="mb-3">
      ...
      >
        <div className="grid grid-cols-[1fr_90px_60px_65px_55px_60px_32px] items-center px-4 py-3 text-sm">
          ...
          <div className="text-center" onClick={(e) => e.stopPropagation()}>
            {isAdmin && !isPast ? (
              <InlineGoalEditor currentGoal={rep.goal} onSave={(g) => onSetRepGoal(rep.clerkUserId, g)} />
            ) : (
...
function RepView({
  ...
  onRemoveTarget: (targetId: string) => Promise<void>;
  spotlightIds?: string[];
  onToggleSpotlight?: (companyId: string) => void;
}) {
  const [toggling, setToggling] = useState<string | null>(null);
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {sortByPriorityThenName(rep.activePartners).map((p) => {
                  ...
                  const isSpotlit = spotlightIds?.includes(p.companyId) ?? false;
                  return (
                    <Fragment key={p.companyId}>
                      ...
                      {isOpen && (
                        <tr>
                          <td colSpan={hasSpotlight ? 7 : 6} className="p-0">
...
function DiscussionSpotlight({
  ...
  const atMax = spotlightIds.length >= 3;

  return (
    <div className="mt-8 pt-6 border-t border-gray-200">
      <div className="flex items-center gap-3 mb-1">
        <h2 className="text-base font-semibold text-gray-900">Discussion Spotlight</h2>
        <span className={cn(
          "text-xs rounded-full px-2 py-0.5 font-medium",
          spotlightIds.length === 3 ? "bg-forest-100 text-forest-700" : "bg-gray-100 text-gray-500"
        )}>
            ...
            <button
              ...
              className={cn(
                ...
                isSelected
                  ...
                  : "border-gray-300 text-gray-600 hover:border-amber-300 hover:text-amber-700 hover:bg-amber-50"
              )}
            >
              <span className={isSelected ? "text-amber-400" : "text-gray-300"}>★</span>
              {c.companyName}
      ...
      {spotlightIds.length > 0 && (
        <div className="space-y-6">
          {spotlightIds.map((id) => {
            const c = myCompanies.find((x) => x.companyId === id);
            if (!c) return null;
            return (
              <div key={id} className="bg-white border-2 border-amber-200 rounded-xl overflow-hidden shadow-sm">
                <div className="flex items-center justify-between px-5 py-3 bg-amber-50 border-b border-amber-100">
                  <div className="flex items-center gap-2">
                    <span className="text-amber-400 text-lg">★</span>
                    <span className="text-base font-semibold text-gray-900">{c.companyName}</span>
                    <PriorityBadge priority={c.priority} />
                    <span className={cn(
                      "text-xs px-2 py-0.5 rounded-full font-medium",
                      c.type === "active" ? "bg-green-100 text-green-700" : "bg-blue-100 text-blue-700"
                    )}>
...
export default function WarRoomTab({ currentUserId, sysRole }: WarRoomTabProps) {
  ...
  const loadPlan = useCallback(async (quarterId: string) => {
    ...
    try {
      ...
      if (data.debug) console.log("[WarRoomTab] debug:", data.debug);
      setPlanData(data);
    } catch (e) {
      setLoadError(String(e));
      console.error("[WarRoomTab] load error:", e);
    } finally {
      setLoading(false);
    }
  }, []);
