...
export default function NurtureTab({ currentUserId, sysRole }: { currentUserId: string; sysRole: string }) {
  ...
  useEffect(() => {
    fetch("/api/crm/quarters")
      .then((r) => r.json())
      .then((data) => {
        // Default to the newest non-archived quarter (API returns them
        // sorted by StartDate desc, so qs[0] is newest) rather than
        // whichever quarter's date range contains today — reps start
        // working a new quarter's pipeline before its start date arrives,
        // so "current" means the latest quarter that exists. Same logic
        // as War Room.
      ...
      {!quartersLoading && quarters.length > 0 && (
        <div>
          <div className="flex items-center gap-3 mb-4 flex-wrap">
            ...
            <div className="flex border border-gray-200 rounded-lg overflow-hidden bg-white">
              {(showArchivedQuarters ? quarters : quarters.filter((q) => !q.isArchived)).map((q) => (
                <button
                  key={q.id}
            ...
            {quarters.some((q) => q.isArchived) && (
              <button onClick={() => setShowArchivedQuarters((s) => !s)}
                className="text-xs text-gray-400 hover:text-gray-600 underline underline-offset-2">
                {showArchivedQuarters ? "Hide Archived" : `Show Archived (${quarters.filter((q) => q.isArchived).length})`}
              </button>
            )}
