...
export default async function PFInventoryPage() {
  ...
  const items = await getItemsByPrimaryRoute("ProFoundFinds Consignment").catch(() => []);
  ...
  await Promise.all(
    uniqueTenantIds.map(async (tenantId) => {
      try {
        ...
        // Find client email: look for non-TTT-admin Owner member in memberships
        let ownerEmail = "";
        try {
          ...
          for (const m of ownerMembers) {
            if (isTTTAdmin(m.userId)) continue; // skip TTT admin accounts
  ...
  return (
    <div className="min-h-screen bg-gray-950">
      <AdminHeader active="pfinventory" />
      <main className="max-w-[1400px] mx-auto px-6 py-8">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-white">PF Inventory</h1>
