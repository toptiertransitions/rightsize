...
const PF_STATUSES: ItemStatus[] = ["Listed", "In Cart", "Sold", "Discarded"];
...
// ─── Bulk edit bar ─────────────────────────────────────────────────────────────
function BulkBar({
  count, onClear, onBulkStatus, onBulkShare, onBulkDelivery, onBulkSite, onPrintLabels,
}: {
  count: number;
  onClear: () => void;
  ...
  onBulkSite: (active: boolean) => void;
  onPrintLabels: () => void;
}) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 mb-4 p-3 bg-forest-900/40 border border-forest-700/50 rounded-xl">
      ...
      <div className="flex items-center gap-1">
        ...
        <button onClick={() => onBulkSite(true)}
          className="px-2 py-1 rounded-full border text-xs font-semibold bg-forest-900/50 text-forest-300 border-forest-700 hover:opacity-80 transition-opacity">
          On
        </button>
        <button onClick={() => onBulkSite(false)}
          className="px-2 py-1 rounded-full border text-xs font-semibold bg-gray-800 text-gray-400 border-gray-700 hover:opacity-80 transition-opacity">
          Off
        </button>
      </div>
    ...
    <div>
      ...
      {selectedIds.length > 0 && (
        <BulkBar
          ...
          onBulkSite={(active) => bulkPatch(selectedIds, () => ({ storefrontActive: active }))}
                ...
                {sorted.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE).map((item, idx) => {
                  ...
                  return (
                    <tr key={item.id} className={`border-b border-gray-800/60 transition-colors ${
                      ...
                      {/* Site (storefrontActive) */}
                      <td className="px-3 py-2.5 text-center">
                        <button
                          onClick={() => patchItem(item.id, item.tenantId, { storefrontActive: !item.storefrontActive })}
                          title={item.storefrontActive ? "On profoundfinds.com — click to remove" : "Not on site — click to add"}
                          className={`w-7 h-7 rounded-md border flex items-center justify-center mx-auto transition-colors ${
                            item.storefrontActive
                              ? "bg-forest-600 border-forest-500 text-white hover:bg-forest-700"
