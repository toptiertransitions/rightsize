...
import type { Item, ItemStatus, VendorDecision, LocalVendor } from "@/lib/types";
import { Pagination } from "../components/Pagination";

const PAGE_SIZE = 25;
const ITEM_STATUSES: ItemStatus[] = ["Listed", "Sold", "Discarded"];
const VENDOR_DECISIONS: VendorDecision[] = ["Pending", "Approved", "Rejected", "Hold"];
...
type SortCol = "itemName" | "status" | "client" | "quantity" | "valueMid" | "salePrice" | "vendorName" | "vendorCut" | "clientSharePercent" | "vendorDecision" | "soldDate";
...
const DECISION_STYLE: Record<string, string> = {
  ...
  Hold:     "bg-blue-900/50 text-blue-300 border-blue-700",
};
  ...
  return (
    <button onClick={start} title="Click to edit" className={`text-left w-full px-2 py-1 rounded hover:bg-gray-700/60 transition-colors group ${className}`}>
      {display
        ...
        : <span className="text-gray-600 text-sm italic group-hover:text-gray-400">{placeholder}</span>
      }
    </button>
  );
}
...
function StatusCell({ value, onSave }: { value: ItemStatus; onSave: (v: ItemStatus) => void }) {
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(o => !o)} className={`px-2 py-1 rounded-full border text-xs font-semibold hover:opacity-80 transition-opacity ${STATUS_STYLE[value] ?? STATUS_STYLE.Discarded}`}>
        {value}
      </button>
      {open && (
        <div className="absolute left-0 top-full mt-1 z-20 bg-gray-800 border border-gray-700 rounded-xl overflow-hidden shadow-xl">
          {ITEM_STATUSES.map(s => (
            <button key={s} onClick={() => { onSave(s); setOpen(false); }}
              className={`block w-full text-left px-4 py-2 text-sm hover:bg-gray-700 transition-colors ${s === value ? "text-white font-semibold" : "text-gray-300"}`}>
              {s}
            </button>
          ))}
        </div>
...
// ─── VendorDecisionCell ───────────────────────────────────────────────────────
function VendorDecisionCell({ value, onSave }: { value?: VendorDecision; onSave: (v: VendorDecision) => void }) {
  const [open, setOpen] = useState(false);
  const display = value ?? "Pending";
  return (
    <div className="relative">
      <button onClick={() => setOpen(o => !o)} className={`px-2 py-1 rounded-full border text-xs font-semibold hover:opacity-80 transition-opacity ${DECISION_STYLE[display] ?? DECISION_STYLE.Pending}`}>
        {display}
      </button>
      {open && (
        <div className="absolute left-0 top-full mt-1 z-20 bg-gray-800 border border-gray-700 rounded-xl overflow-hidden shadow-xl min-w-[120px]">
          {VENDOR_DECISIONS.map(d => (
            <button key={d} onClick={() => { onSave(d); setOpen(false); }}
              className={`block w-full text-left px-4 py-2 text-sm hover:bg-gray-700 transition-colors ${d === value ? "text-white font-semibold" : "text-gray-300"}`}>
              {d}
            </button>
          ))}
        </div>
...
// ─── VendorCell ───────────────────────────────────────────────────────────────
function VendorCell({ vendorId, vendors, onSave }: {
  vendorId?: string; vendors: LocalVendor[]; onSave: (id: string) => void;
}) {
  const [open, setOpen] = useState(false);
  const vendor = vendors.find(v => v.id === vendorId);
  return (
    <div className="relative">
      <button onClick={() => setOpen(o => !o)} title="Click to assign vendor"
        className="text-left w-full px-2 py-1 rounded hover:bg-gray-700/60 transition-colors group">
        {vendor
          ? <span className="text-gray-200 text-xs">{vendor.vendorName}</span>
          : <span className="text-gray-600 text-xs italic group-hover:text-gray-400">Assign vendor</span>
        }
      </button>
      {open && (
        <div className="absolute left-0 top-full mt-1 z-20 bg-gray-800 border border-gray-700 rounded-xl overflow-hidden shadow-xl min-w-[200px] max-h-52 overflow-y-auto">
          <button onClick={() => { onSave(""); setOpen(false); }}
            className="block w-full text-left px-4 py-2 text-xs hover:bg-gray-700 text-gray-500 transition-colors">
            — Unassign
          </button>
          {vendors.map(v => (
            <button key={v.id} onClick={() => { onSave(v.id); setOpen(false); }}
              className={`block w-full text-left px-4 py-2 text-xs hover:bg-gray-700 transition-colors ${v.id === vendorId ? "text-white font-semibold" : "text-gray-300"}`}>
              {v.vendorName}
              {v.consignmentTake > 0 && (
                <span className="ml-1.5 text-amber-400 text-[10px]">{v.consignmentTake}% cut</span>
              )}
            </button>
          ))}
        </div>
...
// ─── BulkBar ──────────────────────────────────────────────────────────────────
function BulkBar({ count, onClear, onBulkStatus, onBulkShare, onBulkVendor, onBulkDecision, onBulkSoldDate, vendors }: {
  count: number; onClear: () => void;
  onBulkStatus: (s: ItemStatus) => void;
  onBulkShare: (pct: number) => void;
  onBulkVendor: (id: string) => void;
  onBulkDecision: (d: VendorDecision) => void;
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 mb-4 p-3 bg-forest-900/40 border border-forest-700/50 rounded-xl">
      ...
      <div className="flex items-center gap-1 relative">
        ...
        <button onClick={() => setVendorOpen(o => !o)}
        ...
        {vendorOpen && (
          <div className="absolute left-0 top-full mt-1 z-30 bg-gray-800 border border-gray-700 rounded-xl overflow-hidden shadow-xl min-w-[200px] max-h-52 overflow-y-auto">
            <button onClick={() => { onBulkVendor(""); setVendorOpen(false); }}
              className="block w-full text-left px-4 py-2 text-xs hover:bg-gray-700 text-gray-500 transition-colors">— Unassign</button>
            {vendors.map(v => (
              <button key={v.id} onClick={() => { onBulkVendor(v.id); setVendorOpen(false); }}
      ...
      <div className="flex items-center gap-1 relative">
        ...
        <button onClick={() => setDecisionOpen(o => !o)}
          className="px-2 py-1 rounded bg-gray-800 border border-gray-600 text-xs text-gray-300 hover:border-gray-400 transition-colors">
          Set ▾
        </button>
        {decisionOpen && (
          <div className="absolute left-0 top-full mt-1 z-30 bg-gray-800 border border-gray-700 rounded-xl overflow-hidden shadow-xl">
            {VENDOR_DECISIONS.map(d => (
              <button key={d} onClick={() => { onBulkDecision(d); setDecisionOpen(false); }}
                className={`block w-full text-left px-4 py-2 text-xs font-semibold hover:opacity-80 transition-colors ${DECISION_STYLE[d]}`}>
                {d}
              </button>
            ))}
          </div>
...
// ─── Main Component ───────────────────────────────────────────────────────────
export function OtherConsignmentClient({ items: initialItems, tenantInfoMap, vendors }: Props) {
  ...
  async function patchItem(id: string, tenantId: string, updates: Partial<Item>) {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) { window.location.href = "/sign-in"; return; }
      const contentType = res.headers.get("content-type") || "";
      if (!contentType.includes("application/json")) throw new Error("Session expired — please reload.");
      const data = await res.json();
  ...
  const filtered = items.filter(item => {
    ...
    const vendor = item.assignedVendorId ? vendorMap.get(item.assignedVendorId) : undefined;
    const haystack = [item.itemName, item.category, tenant?.name, tenant?.ownerEmail, vendor?.vendorName].join(" ").toLowerCase();
    if (search && !haystack.includes(search.toLowerCase())) return false;
    if (statusFilter !== "all" && item.status !== statusFilter) return false;
    if (vendorFilter !== "all" && (vendorFilter === "__unassigned__" ? !!item.assignedVendorId : item.assignedVendorId !== vendorFilter)) return false;
    if (tttFilter !== "all" && (tttFilter === "ttt" ? !(tenant?.isTTT ?? true) : (tenant?.isTTT ?? true))) return false;
    return true;
  });

  const sorted = [...filtered].sort((a, b) => {
    const dir = sortDir === "asc" ? 1 : -1;
    const vA = a.assignedVendorId ? vendorMap.get(a.assignedVendorId) : undefined;
    const vB = b.assignedVendorId ? vendorMap.get(b.assignedVendorId) : undefined;
    switch (sortCol) {
      ...
      case "salePrice":         return dir * ((a.salePrice ?? 0) - (b.salePrice ?? 0));
      case "vendorName":        return dir * ((vA?.vendorName ?? "").localeCompare(vB?.vendorName ?? ""));
      case "vendorCut":         return dir * ((vA?.consignmentTake ?? 0) - (vB?.consignmentTake ?? 0));
      case "clientSharePercent":return dir * ((a.clientSharePercent ?? 0) - (b.clientSharePercent ?? 0));
      case "vendorDecision":    return dir * ((a.vendorDecision ?? "Pending").localeCompare(b.vendorDecision ?? "Pending"));
  ...
  const totalLabelValue = filtered.reduce((s, i) => s + (i.valueMid || 0), 0);
  const soldItems = filtered.filter(i => i.status === "Sold");
  const totalClientPayouts = soldItems.reduce((s, i) => {
    const price = i.salePrice || i.valueMid || 0;
    return s + (price * ((i.clientSharePercent ?? 0) / 100));
  }, 0);

  const assignedVendorIds = new Set(items.map(i => i.assignedVendorId).filter(Boolean));
  const vendorsInUse = vendors.filter(v => assignedVendorIds.has(v.id));
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        {[
          ...
          { label: "Listed",       value: filtered.filter(i => i.status === "Listed").length,   cls: "text-purple-300" },
          { label: "Sold",         value: soldItems.length,                             cls: "text-green-300" },
        ].map(({ label, value, cls }) => (
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl px-4 py-2">
          ...
          <p className="text-lg font-bold text-white">${totalLabelValue.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-xl px-4 py-2">
          <span className="text-xs text-gray-400">Total Client Payouts</span>
          <p className="text-lg font-bold text-amber-300">${totalClientPayouts.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
        </div>
      </div>

      {/* Filters */}
      <div className="flex flex-wrap gap-3 mb-4">
        <input type="text" placeholder="Search items, clients, vendors…" value={search}
        ...
        <select value={vendorFilter} onChange={e => { setVendorFilter(e.target.value); setPage(1); }}
          ...
          {vendorsInUse.map(v => <option key={v.id} value={v.id}>{v.vendorName}</option>)}
        </select>
      ...
      {selectedIds.length > 0 && (
        <BulkBar
          ...
          vendors={vendors}
          onBulkStatus={s => bulkPatch(selectedIds, () => ({ status: s }))}
          onBulkShare={pct => bulkPatch(selectedIds, () => ({ clientSharePercent: pct }))}
          onBulkVendor={id => bulkPatch(selectedIds, () => ({ assignedVendorId: id || undefined }))}
          onBulkDecision={d => bulkPatch(selectedIds, () => ({ vendorDecision: d }))}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-800 bg-gray-800/60">
                  ...
                  <th className="px-3 py-3 text-left w-8"><span className="text-xs font-semibold text-gray-400 uppercase tracking-wide">#</span></th>
                  ...
                  <th className="px-3 py-3 text-left min-w-[180px]">{thBtn("itemName", "Item")}</th>
                  <th className="px-3 py-3 text-left w-24">{thBtn("status", "Status")}</th>
                  <th className="px-3 py-3 text-left min-w-[130px]">{thBtn("client", "Client")}</th>
                  ...
                  <th className="px-3 py-3 text-right w-28">{thBtn("valueMid", "Label Value", "justify-end")}</th>
                  <th className="px-3 py-3 text-right w-28">{thBtn("salePrice", "Sale Price", "justify-end")}</th>
                  <th className="px-3 py-3 text-left min-w-[180px]">{thBtn("vendorName", "Vendor")}</th>
                  <th className="px-3 py-3 text-right w-28">{thBtn("vendorCut", "Consign. Cut", "justify-end")}</th>
                  <th className="px-3 py-3 text-right w-28">{thBtn("clientSharePercent", "Client Share", "justify-end")}</th>
                  <th className="px-3 py-3 text-right w-28"><span className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Client Payout</span></th>
                  <th className="px-3 py-3 text-left w-28">{thBtn("vendorDecision", "Response")}</th>
                  <th className="px-3 py-3 text-left min-w-[160px]"><span className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Vendor Notes</span></th>
              ...
              <tbody>
                {sorted.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE).map((item, idx) => {
                  ...
                  const clientPayout = price && item.clientSharePercent ? (price * (item.clientSharePercent / 100)) : null;
                  ...
                  return (
                    <tr key={item.id} className={`border-b border-gray-800/60 transition-colors ${
                      ...
                      {/* Status */}
                      <td className="px-3 py-2.5">
                        <StatusCell value={item.status as ItemStatus} onSave={v => patchItem(item.id, item.tenantId, { status: v })} />
                      </td>
                      ...
                      <td className="px-3 py-2.5">
                        {tenant ? (
                          <div>
                            <p className="text-gray-300 text-xs font-medium truncate max-w-[120px]">{tenant.name}</p>
                            <p className="text-gray-500 text-[10px] truncate max-w-[120px]">{tenant.ownerEmail}</p>
                          </div>
                      ...
                      <td className="px-1 py-1 text-right">
                        <EditCell value={item.quantity || 1} type="number"
                          onSave={v => patchItem(item.id, item.tenantId, { quantity: Number(v) || 1 })}
                      ...
                      <td className="px-1 py-1 text-right">
                        <EditCell value={item.valueMid || undefined} type="number" prefix="$"
                          ...
                          placeholder="—" className="text-right font-semibold" />
                      </td>

                      {/* Sale Price */}
                      <td className="px-1 py-1 text-right">
                        <EditCell value={item.salePrice || undefined} type="number" prefix="$"
                          onSave={v => patchItem(item.id, item.tenantId, { salePrice: Number(v) || 0 })}
                          placeholder="—" className="text-right" />
                      </td>

                      {/* Vendor */}
                      <td className="px-1 py-1">
                        <VendorCell vendorId={item.assignedVendorId} vendors={vendors}
                          onSave={id => patchItem(item.id, item.tenantId, { assignedVendorId: id || undefined })} />
                      </td>

                      {/* Consignment Cut % (from vendor directory) */}
                      <td className="px-3 py-2.5 text-right">
                        {vendor && vendor.consignmentTake > 0
                          ? <span className="text-amber-300 text-xs font-semibold">{vendor.consignmentTake}%</span>
                          : <span className="text-gray-600 text-xs">—</span>
                        }
                      </td>
                      ...
                      <td className="px-3 py-2.5 text-right">
                        {clientPayout != null
                          ? <span className="text-green-400 text-xs font-semibold">${clientPayout.toFixed(2)}</span>
                          : <span className="text-gray-600 text-xs">—</span>
                        }
                      </td>
                      ...
                      {/* Vendor Response */}
                      <td className="px-3 py-2.5">
                        <VendorDecisionCell value={item.vendorDecision}
                          onSave={d => patchItem(item.id, item.tenantId, { vendorDecision: d })} />
                      </td>

                      {/* Vendor Notes */}
                      <td className="px-1 py-1">
                        <EditCell value={item.vendorNotes ?? undefined}
                          onSave={v => patchItem(item.id, item.tenantId, { vendorNotes: v })}
                      ...
                      <td className="px-1 py-1">
                        <EditCell
                          value={item.saleDate ? item.saleDate.slice(0, 10) : undefined}
                          type="date"
                          onSave={v => patchItem(item.id, item.tenantId, { saleDate: v || undefined })}
                          placeholder={fmtDate(item.saleDate) || "—"}
                        />
                      </td>
                    </tr>
                  );
      ...
      <p className="text-xs text-gray-600 mt-3">
        Click any cell to edit inline. Consignment Cut % is pulled from the vendor directory for reference. Client Payout = Sale Price (or Label Value) × Client Share %.
      </p>
    </div>
  );
}
