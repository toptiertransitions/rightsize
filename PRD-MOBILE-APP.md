...
| **OD-5** | App name and bundle ID | **Decided.** Bundle ID: `com.toptiertransitions.rightsizeapp` (not `...rightsize` — that string got claimed against the Apple ID during earlier Personal Team testing, before the paid Organization account existed, and Apple won't release it). On-device display name: "Rightsize". App Store Connect public listing name: **"Rightsize - Move Management"** (27 characters, fits Apple's 30-character cap). | Ready for the App Store Connect app record |
