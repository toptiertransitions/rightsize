...
function isPdf(file: ProjectFile) {
  return file.resourceType === "raw" || file.fileName.toLowerCase().endsWith(".pdf");
}

// Same proxy-download approach as VendorFilesSection — PDFs are served
// through a server-signed URL so they open/download correctly regardless
// of how Cloudinary stored them (raw vs image resource type).
function fileDownloadUrl(file: ProjectFile): string {
  if (isPdf(file)) return `/api/files/download?id=${file.id}`;
...
// File management for the currently-selected partner on a category card —
// same add/delete/download-with-original-filename pattern as the Vendors
// page's VendorFilesSection, reusing the identical /api/files endpoints and
// "Vendor File" tag so no backend or permission changes were needed.
// Scoped by (tenantId, partnerId) so one client's files with a shared
// marketplace partner are never visible to another client who picked the
// same partner.
export function PartnerFilesSection({ tenantId, partnerId }: Props) {
  ...
  const handleFiles = async (fileList: FileList) => {
    ...
    const newUploading: UploadingFile[] = items.map((f) => ({ id: Math.random().toString(36).slice(2), name: f.name }));
  ...
  return (
    <div className="border-t border-gray-100 mt-1 pt-1">
      <button
        ...
        className="w-full flex items-center justify-between px-1 pt-2 pb-1 text-xs font-medium text-gray-400 hover:text-gray-600 transition-colors"
      >
        <span className="flex items-center gap-1.5">
          <svg className={`w-3 h-3 transition-transform ${expanded ? "rotate-0" : "-rotate-90"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
          </svg>
          Files{loaded ? ` (${totalCount})` : ""}
        </span>
      </button>
      ...
      {expanded && (
        <div className="pt-1 pb-1 space-y-1.5">
          {files.map((file) => (
            <div key={file.id} className={`flex items-center gap-2 group transition-opacity ${deletingIds.has(file.id) ? "opacity-40" : ""}`}>
              {isPdf(file) ? (
                <PdfIcon />
              ) : (
                <div className="w-8 h-8 flex-shrink-0 rounded overflow-hidden border border-gray-100 bg-gray-50">
                  <Image src={file.cloudinaryUrl} alt={file.fileName} width={32} height={32} className="w-full h-full object-cover" unoptimized />
                </div>
              )}
              <a
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs text-forest-600 hover:text-forest-700 font-medium mt-1 px-1 min-h-[28px]"
          >
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
