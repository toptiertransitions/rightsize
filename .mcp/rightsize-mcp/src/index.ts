...
if (!AT_TOKEN || !AT_BASE) {
  process.stderr.write(
    "ERROR: AIRTABLE_API_TOKEN and AIRTABLE_BASE_ID must be set in environment\n"
  );
  process.exit(1);
}
...
interface ARecord { id: string; fields: Fields; createdTime?: string; }

const AT_HEADERS: Record<string, string> = {
...
async function atFetch(path: string, init?: RequestInit): Promise<Response> {
  return fetch(`https://api.airtable.com/v0/${AT_BASE}${path}`, {
    ...init,
    headers: { ...AT_HEADERS, ...(init?.headers as Record<string,string> ?? {}) },
  });
}
...
const daysAgo   = (d: number): string => { const dt = new Date(); dt.setDate(dt.getDate() - d); return dt.toISOString().slice(0, 10); };
const daysAhead = (d: number): string => { const dt = new Date(); dt.setDate(dt.getDate() + d); return dt.toISOString().slice(0, 10); };
...
function bestStage(stages: string[]): string {
  if (stages.length === 0) return "Identified";
  return stages.reduce((best, st) =>
    (STAGE_PRIORITY[st] ?? 0) > (STAGE_PRIORITY[best] ?? 0) ? st : best,
    stages[0]
  );
}
...
async function crmDashboard(): Promise<string> {
  ...
  const overdueNextSteps: Array<{ id: string; clientName: string; stage: string; nextStep: string; dueDate: string }> = [];
  const dueThisWeek:      Array<{ id: string; clientName: string; stage: string; nextStep: string; dueDate: string }> = [];
  ...
  const staleReferrals = contacts
    .filter(c => {
      ...
      if (stage === "Inactive Referral") return false;
  ...
  const result = companies
    ...
    .filter(co => {
      ...
      if (search && !co.name.toLowerCase().includes(search) &&
          ...
          !co.type.toLowerCase().includes(search)) return false;
      return true;
    });
...
async function searchContacts(args: Fields): Promise<string> {
  ...
  type Match = { type: string; id: string; name: string; email: string; phone: string; company?: string; stage?: string; title?: string; source?: string };
  const matches: Match[] = [];
...
const TOOLS = [
  {
    name: "crm_dashboard",
    description:
      "Morning briefing: open opportunity pipeline by stage, overdue and upcoming next steps, " +
  ...
  },
  {
    ...
    description:
      "List all referral partner companies with their contacts, best relationship stage, " +
  ...
  },
  {
    ...
    description:
      "Client opportunity pipeline. Returns opportunities sorted by next step date ascending " +
  ...
  },
  {
    ...
    description:
      "Search for a person by name, email, or job title. Returns matching referral contacts " +
  ...
  },
  {
    ...
    description:
      "Recent CRM activity log — calls, emails, meetings, notes, tasks. " +
  ...
  },
  {
    name: "get_projects",
    description:
      "List active client projects. Use to see what's currently in flight, " +
  ...
  },
  {
    ...
    description:
      "Add a new referral partner company to the CRM. After creating, " +
  ...
  },
  {
    ...
    description:
      "Add a contact person at a referral partner company. " +
  ...
  },
  {
    ...
    description:
      "Create a new client lead. Adds both a ClientContact record and an Opportunity in Lead stage. " +
  ...
  },
  {
    name: "log_activity",
    description:
      "Log a CRM activity — call, email, meeting, note, or task — against an opportunity or contact. " +
  ...
  },
  {
    ...
    description:
      "Permanently delete a CRM activity. DESTRUCTIVE — cannot be undone. " +
...
const server = new Server(
  { name: "rightsize-mcp", version: "1.0.0" },
  { capabilities: { tools: {} } }
);
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  const a = (args ?? {}) as Fields;

  try {
    let result: string;
    switch (name) {
      case "crm_dashboard":          result = await crmDashboard();                break;
      case "get_referral_partners":  result = await getReferralPartners(a);        break;
      case "get_opportunities":      result = await getOpportunities(a);           break;
      case "search_contacts":        result = await searchContacts(a);             break;
      case "get_recent_activities":  result = await getRecentActivities(a);        break;
      case "get_projects":           result = await getProjects(a);                break;
      case "create_referral_company":result = await createReferralCompany(a);      break;
      case "create_referral_contact":result = await createReferralContact(a);      break;
