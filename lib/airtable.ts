...
function mapTenant(record: Airtable.Record<Airtable.FieldSet>): Tenant {
  ...
  return {
    ...
    originSqFt: f["OriginSqFt"] != null ? toNum(f["OriginSqFt"]) : undefined,
    originHighSqFt: f["OriginHighSqFt"] != null ? toNum(f["OriginHighSqFt"]) : undefined,
    originMedSqFt: f["OriginMedSqFt"] != null ? toNum(f["OriginMedSqFt"]) : undefined,
    originLowSqFt: f["OriginLowSqFt"] != null ? toNum(f["OriginLowSqFt"]) : undefined,
...
export async function getUserByClerkId(clerkUserId: string): Promise<User | null> {
  ...
  const user = mapUser(records[0]);
  // Fire-and-forget: stamp LastActivityAt so we can observe actual usage before removing this table.
  touchUserLastActivity(records[0].id).catch(() => {});
  return user;
}
...
async function touchUserLastActivity(airtableId: string): Promise<void> {
  const base = getBase();
  await base(AIRTABLE_TABLES.USERS).update(airtableId, {
...
function mapUser(record: Airtable.Record<Airtable.FieldSet>): User {
  ...
  return {
    ...
    name: toStr(f["Name"]),
    createdAt: toStr(f["CreatedAt"]),
    lastActivityAt: toStr(f["LastActivityAt"]) || undefined,
  };
}
...
export async function getTimeEntriesForTenants(tenantIds: string[]): Promise<TimeEntry[]> {
  if (tenantIds.length === 0) return [];
  // Sequential per-tenant filtered fetches: each call returns only that tenant's entries
  // (no full-table scan, no pagination for most projects), and a single failure only
  // drops one project rather than zeroing out all logged hours.
  const results: TimeEntry[] = [];
  for (const id of tenantIds) {
    const entries = await getTimeEntries({ tenantId: id }).catch(() => []);
...
export async function getItemsByIds(ids: string[]): Promise<Item[]> {
  ...
  const records = await base(AIRTABLE_TABLES.ITEMS).select({ filterByFormula: formula }).all();
  return records.map(mapItem);
}
...
export async function updateItem(
  ...
  const fieldMap: Record<keyof Item, string> = {
    ...
    alsoListedOnline: "AlsoListedOnline",
    // Shipping label
...
export async function getStorefrontBuyersByEmail(email: string): Promise<import("./types").StorefrontBuyer[]> {
  const base = getBase();
  const records: Airtable.Record<Airtable.FieldSet>[] = [];
  const safeEmail = email.toLowerCase().trim().replace(/"/g, "");
  await base(AIRTABLE_TABLES.STOREFRONT_BUYERS)
    .select({ filterByFormula: `LOWER(TRIM({BuyerEmail})) = "${safeEmail}"`, sort: [{ field: "CreatedAt", direction: "desc" }] })
    .eachPage((page, next) => { records.push(...page); next(); });
  return records.map(r => {
    const f = r.fields;
    return {
...
function mapItem(record: Airtable.Record<Airtable.FieldSet>): Item {
  ...
  return {
    ...
    primaryRoute: toStr(f["PrimaryRoute"]) as PrimaryRoute || undefined,
    ...
    alsoListedOnline: !!f["AlsoListedOnline"],
...
export async function updateTenant(
  ...
  if (data.originSqFt !== undefined) fields["OriginSqFt"] = data.originSqFt;
  if (data.originHighSqFt !== undefined) fields["OriginHighSqFt"] = data.originHighSqFt;
  if (data.originMedSqFt !== undefined) fields["OriginMedSqFt"] = data.originMedSqFt;
  if (data.originLowSqFt !== undefined) fields["OriginLowSqFt"] = data.originLowSqFt;
...
function mapClientContact(record: AirtableRecord): ClientContact {
  ...
  return {
    ...
    staffReferralId: toStr(f["StaffReferralId"]) || undefined,
    notes: toStr(f["Notes"]),
    assignedToClerkId: toStr(f["AssignedToClerkId"]),
    createdAt: toStr(f["CreatedAt"]),
    tags: toStr(f["Tags"]) || undefined,
  ...
  const res = await crmFetch(AIRTABLE_TABLES.CRM_CLIENT_CONTACTS, "", {
    ...
    body: JSON.stringify({
      fields: {
        ...
        StaffReferralId: data.staffReferralId || "",
        Notes: data.notes || "",
        AssignedToClerkId: data.assignedToClerkId || "",
        CreatedAt: new Date().toISOString(),
      },
...
export async function updateClientContact(
  id: string,
  data: Partial<{ name: string; email: string; phone: string; source: string; referralPartnerId: string; clientReferralId: string; staffReferralId: string; notes: string; assignedToClerkId: string }>
): Promise<ClientContact> {
  const fields: Record<string, unknown> = {};
  if (data.name !== undefined) fields["Name"] = data.name;
  if (data.email !== undefined) fields["Email"] = data.email;
  ...
  if (data.staffReferralId !== undefined) fields["StaffReferralId"] = data.staffReferralId;
  if (data.notes !== undefined) fields["Notes"] = data.notes;
  if (data.assignedToClerkId !== undefined) fields["AssignedToClerkId"] = data.assignedToClerkId;
  const res = await crmFetch(AIRTABLE_TABLES.CRM_CLIENT_CONTACTS, `/${id}`, {
    method: "PATCH",
